...
```bash
# 从上次同步的提交到目标 Release Tag
# （上次同步的 HEAD 从历史同步记录表获取）
git log --oneline <LAST_SYNCED_COMMIT>..<TARGET_TAG> > /tmp/cherry-pick-commits.txt
wc -l /tmp/cherry-pick-commits.txt   # 查看总数
```

也可以先用 tags 确认可选的同步目标：
```bash
git tag -l "v2.*"   # 列出所有 upstream 版本 tags
```
...
| 日期 | 同步范围 | 同步至 Tag | 提交数 | 冲突数 | HEAD | 变更摘要 | 文档 |
|------|---------|-----------|--------|--------|------|---------|------|
