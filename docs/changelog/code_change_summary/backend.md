...
## 15. 2026-06-04: WeCom Bot 渠道配置

### 后端变更

| 文件 | 变更类型 | 说明 |
|------|---------|------|
| `deerflow_extensions/env_settings/router.py` | 修改 | 路径拆分为 `/providers` + `/channels`，新增 filelock，新增 4 个渠道端点 |
...
## 16. 2026-06-08: filelock 正式依赖声明

### 问题
`env_settings` 扩展使用 `filelock` 保护 `.env` 并发写入，但 `filelock` 未声明为正式依赖，仅作为传递依赖存在。新环境缺少该模块时，`env_settings` 扩展静默加载失败，所有 `/api/env-settings/*` 路由未注册。

### 修改
| 文件 | 变更类型 | 说明 |
|------|---------|------|
| `backend/pyproject.toml` | 修改 | dependencies 追加 `"filelock>=3.0.0"` |

### 影响
- `uv sync` / Docker build / PyInstaller 自动携带 filelock
- 无需在部署服务器上手动安装

---

## 17. 2026-06-08: config.yaml 渠道 enabled 自动管理

### 问题
渠道配置保存后，用户仍需手动编辑 `config.yaml` 设置 `channels.wecom.enabled: true` 并重启服务才能生效，与 API Key 配置的"保存即用"体验不一致。

### 修改
| 文件 | 变更类型 | 说明 |
|------|---------|------|
| `deerflow_extensions/env_settings/router.py` | 修改 | 新增 `_set_channel_enabled_in_config()`，PUT 时自动设 enabled=true，DELETE 时自动设 enabled=false |

### 验证
- PUT /channels → config.yaml 自动设 `enabled: true`
- DELETE /channels/{channel} → config.yaml 自动设 `enabled: false`
- config.yaml 写入失败不影响凭据保存和渠道重启（非关键路径）

---

## 18. 2026-06-08: 渠道凭据模型通用化 + 多渠道支持

### 问题
渠道凭据模型（`ChannelInfo.bot_id_exists`/`bot_id_masked`/`bot_secret_exists`）是 WeCom 特化的，新增渠道（飞书、钉钉、微信）需通用凭据模型。同时 PUT/DELETE/verify handler 中存在 `"wecom"` 硬编码，不支持多渠道分发。

### 后端修改

| 文件 | 变更类型 | 说明 |
|------|---------|------|
| `deerflow_extensions/env_settings/router.py` | 修改 | `ChannelInfo`/`ChannelUpdateRequest`/`ChannelVerifyRequest` 改为通用 `credentials` 字典；`_CHANNEL_META` 扩展支持 feishu/dingtalk/wechat；新增 `_test_feishu_connect`/`_test_dingtalk_connect`/`_test_wechat_connect` 连通性测试函数；`_channel_test_fns` 分发字典 |

### 前端修改

| 文件 | 变更类型 | 说明 |
|------|---------|------|
| `frontend/extensions/env-settings/channels.ts` | 新增 | 4 个国内 IM 渠道元数据定义（wecom/feishu/dingtalk/wechat），含 `credentialFields` 凭据字段描述 |
| `frontend/extensions/env-settings/types.ts` | 修改 | `ChannelInfo` `credentials` 字典化；新增 `ChannelVerifyRequest` |
| `frontend/extensions/env-settings/api.ts` | 修改 | `verifyChannel()` 参数改为 `credentials` 字典 |
| `frontend/extensions/env-settings/hooks.ts` | 修改 | `useVerifyChannel` 适配新签名 |
| `frontend/extensions/env-settings/channel-settings-page.tsx` | 修改 | 多渠道选择器 + 动态凭据表单渲染 |

### 核心改动

- **后端的凭据字典化**：`ChannelUpdateRequest.bot_id`/`bot_secret` 合并为 `credentials: dict[str, str]`，新增渠道无需改路由参数结构
- **后端连通性测试分发**：`_channel_test_fns` 字典根据 `channel_id` 分发到 `_test_wecom_connect`/`_test_feishu_connect`/`_test_dingtalk_connect`/`_test_wechat_connect`
