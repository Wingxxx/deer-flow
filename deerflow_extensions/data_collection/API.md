...
#### `record(sample_type: str, data: dict, identity: dict | None = None) -> None`
...
**Identity injection:** When `identity` is provided, its keys (`user_id`, `channel_user_id`) are merged into `data` before writing. The middleware passes identity as an explicit argument via the `identity=` parameter (not via shared mutable state). Identity keys are only present when:
...
- The caller explicitly passes an `identity` dict

- `sample_type` -- One of: `agent_input`, `model_output`, `tool_call_request`, `tool_call_result`, `agent_intermediate_state`, `final_response`.
- `data` -- Arbitrary JSON-serializable payload. Identity fields (`user_id`, `channel_user_id`) are auto-injected from the `identity` parameter when provided.
- `identity` -- Optional dict with `user_id` and/or `channel_user_id`. When provided, merged into `data` before None filtering. Keys conflicting with `data` keys emit a WARNING and identity overrides.

---

#### `record_agent_input(session_id, user_query, system_prompt, history_context, rag_context="", agent_config=None, identity=None) -> None`
...
- `identity` -- Optional dict with `user_id` and/or `channel_user_id` (default: `None`).

---

#### `record_model_output(session_id, step_number, raw_response, response_type, finish_reason, tool_calls, token_usage, thinking_content, latency_ms, identity=None) -> None`
...
#### `record_tool_call(session_id, step_number, tool_name, tool_params, call_id, tool_result=None, error=None, duration_ms=0.0, phase="request", identity=None) -> None`
...
#### `record_intermediate_state(session_id, step_number, total_steps, message_count, accumulated_tokens, tools_called, loop_detected=False, identity=None) -> None`
...
#### `record_final_response(session_id, final_response, total_duration_ms, total_llm_calls, total_tool_calls, total_tokens, resolution_status, identity=None) -> None`
...
**Identity flow**: Identity is extracted in `before_model` via `_prepare_identity_for_collector()` (returns `dict | None`) and passed explicitly as `identity=` to `collector.record_*()` methods. Downstream hooks restore cached identity via `_restore_identity_for_collector()` (returns a defensive copy, `dict | None`). No shared mutable state — no `_current_identity` attribute.

**Fail-closed startup**: If `load_config()` raises during `__init__`, all identity collection is disabled (`_collect_user_identity=False`, `_collect_channel_user_id=False`, `_pseudonymize_identity=False`) to prevent silent plaintext leakage.
...
**Auto-salt**: When `pseudonymize_identity=true` and `pseudonym_salt` is empty, `_load_or_create_salt()` is called to generate a random salt via `secrets.token_hex(32)` and persist it to `{output_dir}/.pseudonym_salt`.

**Plaintext gate**: When `collect_user_identity=true` and `pseudonymize_identity=false`, raises `ValueError` unless `DATA_COLLECTION_ALLOW_PLAINTEXT_IDENTITY=true` is set.

---

### `_load_or_create_salt(output_dir: str) -> str`

```python
def _load_or_create_salt(output_dir: str) -> str
```

Load pseudonym salt from `{output_dir}/.pseudonym_salt`, or create and persist a new one via `secrets.token_hex(32)`.

**Priority**: `DATA_COLLECTION_PSEUDONYM_SALT` env var (applied before this function) > persisted file > auto-generate.

- If the file exists and contains ≥32 characters of salt, returns its content.
- Otherwise, generates a 64-character hex salt, persists to file, and returns it.
- If file persistence fails, logs a WARNING and returns the in-memory salt (salt will not survive restart).
- Returns a 64-character hex string with 256 bits of entropy.
...
DEFAULT_CONFIG: dict[str, Any] = {
    ...
    "pseudonym_salt": "",  # Auto-generated via secrets.token_hex(32) when empty
}
```

Default configuration values used when no external config source is available. The `pseudonym_salt` is auto-generated at runtime via `_load_or_create_salt()`.
