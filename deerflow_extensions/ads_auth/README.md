...
用户输入 ADS 账号密码 → POST /api/v1/auth/login/ads
  ...
  → 写入 credentials 到 ADS-MCP 的 config.json
...
| `token_manager.py` | 内存 token 存储 + 写 credentials 到 MCP config.json |
...
登录成功后自动写入以下字段（仅 `credentials.new` 和 `server.url`，不再写入 token）：
